...
public actor OrpheusTTS {
    ...
    private static let startToken = 128259
    private static let endToken = 128258
    private static let padToken = 128263
    private static let audioStartToken = 128261
    private static let audioEndToken = 128262
    private static let voicePrefixToken = 128260
    private static let repetitionContextSize = 20
    private static let codeOffset = 128266
    private static let audioCodeDataStartMarker = 128257
    ...
    private init(weights: [String: MLXArray], snacDecoder: SNACDecoder, tokenizer: OrpheusTokenizer, layers: [OrpheusTransformerBlock]) {
        ...
        while i < maxOutputTokens {
            ...
            if Profiler.enabled {
                ...
                if i < 5 || i % 10 == 0 {
                    ...
                    Log.perf.debug("    ❓ Unaccounted: \(String(format: "%.2f", unaccountedTime))ms")
    ...
    private func forward(inputIds: MLXArray, currentKvCaches: [Cache?]) -> (logits: MLXArray, updatedKvCaches: [Cache?]) {
        ...
        if Profiler.enabled && (!isCaching || (isCaching && inputIds.shape[1] % 50 == 0)) {
            ...
            Log.tts.debug("Generated tokens count: \(tokenCount)")
        }
        ...
        for i in 0..<self.layers.count {
            ...
            if Profiler.enabled {
                ...
                if (!isCaching || currentKvCaches.first??.offset ?? 0 < 50) && (i < 3 || i % 5 == 0) {
                    Log.perf.debug("  🧠 Layer \(i): \(String(format: "%.2f", layerDuration))ms")
                }
            }
        }
        ...
        if Profiler.enabled {
            ...
            if !isCaching || (currentKvCaches.first??.offset ?? 0) < 50 {
                Log.perf.debug("  🧠 All layers: \(String(format: "%.2f", layersTotalDuration))ms")
            }
        }

        // 3. Final RMSNorm
        guard let finalNormWeight = weights["model.norm.weight"] else {
            Log.model.error("Final norm weight not found.")
        ...
        if Profiler.enabled {
            ...
            Log.perf.debug("  ➡️ Forward pass total: \(String(format: "%.2f", forwardDuration))ms")
        }
