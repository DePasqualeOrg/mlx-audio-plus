...
// Voice files are bundled with the app as JSON files (converted from PyTorch .pt files)
class VoiceLoader {
  private init() {}

  static var availableVoices: [TTSVoice] {
    Array(VoiceFileMap.voiceFiles.keys)
  }

  static func loadVoice(_ voice: TTSVoice) -> MLXArray {
    let (file, ext) = VoiceFileMap.voiceFiles[voice]!
    guard let filePath = Bundle.main.path(forResource: file, ofType: ext) else {
      fatalError("Voice file \(file).\(ext) not found in bundle")
    }
...
extension TTSVoice {
  static func fromIdentifier(_ identifier: String) -> TTSVoice? {
    let reverseMapping = Dictionary(
      VoiceLoader.VoiceFileMap.voiceFiles.map { (voice, fileInfo) in
        (fileInfo.0, voice)
      },
