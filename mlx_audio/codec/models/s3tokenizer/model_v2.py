...
def precompute_freqs_cis(
    ...
    Note: This is a custom RoPE implementation specific to S3Tokenizer.
    It cannot be replaced with mx.fast.rope because:
    1. Uses [cos, cos] and [sin, sin] concatenation pattern (not interleaved)
    2. Uses rotation formula [-half_r, half_l] which differs from standard RoPE
    3. Applied before head transpose, while mx.fast.rope expects (B, n_head, T, D)
    """
    freqs = 1.0 / (
...
def apply_rotary_emb(
    ...
    This uses the S3Tokenizer-specific rotation pattern where the vector is
    split at the midpoint and rotated as [-right_half, left_half].
    """
...
class FSQCodebook(nn.Module):
    ...
    Custom quantization using tanh + rounding to discretize features into
    3^8 = 6561 codes. Projects 1280-dim features to 8-dim, applies tanh
    scaling, rounds to {-1, 0, 1}, then encodes as base-3 integer.
    """
...
class FSMNMultiHeadAttention(MultiHeadAttention):
    ...
    Combines standard scaled dot-product attention with a depthwise conv1d
    memory block. The FSMN block processes the value tensor with a kernel
    size of 31, adding local context to the attention output.

    Uses mx.fast.scaled_dot_product_attention for the attention computation.
    """
    ...
    def __init__(
        ...
        # Depthwise conv for local memory - groups=n_state makes it depthwise
    ...
    def forward_fsmn(
        ...
        x_padded = mx.pad(
            inputs, [(0, 0), (self.left_padding, self.right_padding), (0, 0)]
        )
    ...
    def qkv_attention(
        ...
        B, T, D = q.shape
        # Scale by (d_k)^(-0.25) on both q and k = (d_k)^(-0.5) total = 1/sqrt(d_k)
        ...
        # Apply rotary embeddings before transpose (S3Tokenizer-specific order)
        if freqs_cis is not None:
            cos, sin = freqs_cis
            q, k = apply_rotary_emb(q, k, cos[:T], sin[:T])

        # FSMN memory from depthwise conv on values
        ...
        # scale=1 since we pre-scaled q and k above
