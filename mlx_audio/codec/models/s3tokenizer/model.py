...
def sinusoids(length: int, channels: int, max_timescale: float = 10000) -> mx.array:
    ...
    V1 uses fixed sinusoidal positional embeddings (like original Transformer),
    unlike V2/V3 which use rotary embeddings.
    """
    assert channels % 2 == 0
...
class EuclideanCodebook(nn.Module):
    ...
    V1 uses L2-normalized Euclidean distance quantization with 4096 codes,
    unlike V2/V3 which use Finite Scalar Quantization (FSQ) with 6561 codes.
...
class VectorQuantization(nn.Module):
    ...
    def encode(self, x: mx.array) -> mx.array:
        # L2 normalize before quantization (epsilon added for numerical stability)
