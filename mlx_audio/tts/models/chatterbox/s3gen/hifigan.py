...
def hann_window_periodic(size: int) -> mx.array:
    ...
    """
    return mx.array(
        [0.5 * (1 - math.cos(2 * math.pi * n / size)) for n in range(size)]
    )
...
def istft(magnitude: mx.array, phase: mx.array, n_fft: int, hop_length: int, window: mx.array) -> mx.array:
    """
...
class HiFTGenerator(nn.Module):
    ...
    def _istft(self, magnitude: mx.array, phase: mx.array) -> mx.array:
        ...
        return istft(magnitude, phase, self.istft_params["n_fft"], self.istft_params["hop_len"], self.stft_window)
