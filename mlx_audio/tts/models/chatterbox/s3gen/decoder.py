    ...
    def __init__(
        ...
        for i, ch in enumerate(channels):
            ...
            if not is_last:
                ...
                downsample = CausalConv1d(output_channel, output_channel, 3) if causal else nn.Conv1d(output_channel, output_channel, 3, padding=1)

            setattr(self, f'down_blocks_{i}', DownBlock(resnet, transformer_blocks, downsample))
        ...
        for i in range(len(channels_reversed) - 1):
            ...
            if not is_last:
                ...
                upsample = CausalConv1d(output_channel, output_channel, 3) if causal else nn.Conv1d(output_channel, output_channel, 3, padding=1)

            setattr(self, f'up_blocks_{i}', UpBlock(resnet, transformer_blocks, upsample))
    ...
    def __call__(
        ...
        if spks is not None:
            spks_expanded = mx.broadcast_to(
                ...
                (spks.shape[0], spks.shape[1], x.shape[2])
            )
