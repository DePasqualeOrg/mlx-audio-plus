...
from .flow_matching import CFM_PARAMS
...
class S3Token2Wav(S3Token2Mel):
    ...
    def sanitize(self, weights: Dict[str, mx.array]) -> Dict[str, mx.array]:
        ...
        for base_key, pair in wn_pairs.items():
            if "g" in pair and "v" in pair:
                ...
                v = pair["v"]  # weight, shape (out_ch, in_ch, kernel) or (out_ch, kernel, in_ch) for ConvT
        ...
        for key, value in non_wn_weights.items():
            ...
            new_key = re.sub(r'down_blocks\.(\d+)\.0\.', r'down_blocks_\1.resnet.', new_key)
            # down_blocks.X.1.Y -> down_blocks_X.transformer_Y (second tuple element is transformer list)
            new_key = re.sub(r'down_blocks\.(\d+)\.1\.(\d+)\.', r'down_blocks_\1.transformer_\2.', new_key)
            # down_blocks.X.2 -> down_blocks_X.downsample (third tuple element is downsample)
            new_key = re.sub(r'down_blocks\.(\d+)\.2\.', r'down_blocks_\1.downsample.', new_key)

            # mid_blocks.X.0 -> mid_blocks_X.resnet
            new_key = re.sub(r'mid_blocks\.(\d+)\.0\.', r'mid_blocks_\1.resnet.', new_key)
            # mid_blocks.X.1.Y -> mid_blocks_X.transformer_Y
            new_key = re.sub(r'mid_blocks\.(\d+)\.1\.(\d+)\.', r'mid_blocks_\1.transformer_\2.', new_key)
            ...
            new_key = re.sub(r'up_blocks\.(\d+)\.1\.(\d+)\.', r'up_blocks_\1.transformer_\2.', new_key)
            # up_blocks.X.2 -> up_blocks_X.upsample
            new_key = re.sub(r'up_blocks\.(\d+)\.2\.', r'up_blocks_\1.upsample.', new_key)
            ...
            new_key = re.sub(r'\.downsample\.(weight|bias)$', r'.downsample.conv.\1', new_key)
            new_key = re.sub(r'\.upsample\.(weight|bias)$', r'.upsample.conv.\1', new_key)

            # === Final block naming ===
            new_key = new_key.replace('.final_block.block.0.', '.final_block.conv.conv.')
            ...
            has_pytorch_indices = any('.condnet.6.' in k or '.condnet.8.' in k for k in weights.keys())
    ...
    def __call__(
        ...
        output_mels = super().__call__(speech_tokens, ref_dict=ref_dict, finalize=finalize)
    ...
    def inference(
        ...
        """
        output_mels = self.flow_inference(speech_tokens, ref_dict=ref_dict, finalize=finalize)
