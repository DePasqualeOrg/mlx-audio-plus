...
def kaldi_fbank(
    ...
    if win_length < n_fft:
        ...
        frames = mx.concatenate([frames, mx.zeros((frames.shape[0], pad_amount))], axis=1)
...
class CAMPPlus(nn.Module):
    ...
    def sanitize(self, weights: dict) -> dict:
        ...
        for key, value in weights.items():
            ...
            new_key = re.sub(r'xvector\.block(\d+)\.', lambda m: f'blocks.{int(m.group(1))-1}.', new_key)
            # xvector.transit1 -> transits.0, etc.
            new_key = re.sub(r'xvector\.transit(\d+)\.', lambda m: f'transits.{int(m.group(1))-1}.', new_key)
            ...
            new_key = re.sub(r'\.tdnnd(\d+)\.', lambda m: f'.layers.{int(m.group(1))-1}.', new_key)

            # === NonLinear structure ===
            # .nonlinear1.batchnorm. -> .nonlinear1.0. (BatchNorm in Sequential)
            new_key = re.sub(r'\.nonlinear(\d+)\.batchnorm\.', r'.nonlinear\1.0.', new_key)
            ...
            if new_key.startswith('out_nonlinear.batchnorm.'):
                new_key = new_key.replace('out_nonlinear.batchnorm.', 'out_nonlinear.0.', 1)
