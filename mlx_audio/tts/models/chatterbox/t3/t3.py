...
from ..config import T3Config, LLAMA_CONFIGS
...
class T3(nn.Module):
    ...
    def sanitize(self, weights: Dict[str, mx.array]) -> Dict[str, mx.array]:
        ...
        if hasattr(self.tfmr, 'sanitize'):
            tfmr_weights = {k: v for k, v in new_weights.items() if k.startswith("tfmr.")}
            other_weights = {k: v for k, v in new_weights.items() if not k.startswith("tfmr.")}
    ...
    def prepare_input_embeds(
        ...
        if cond_emb.shape[0] != text_emb.shape[0]:
            cond_emb = mx.broadcast_to(
                cond_emb,
                (text_emb.shape[0],) + cond_emb.shape[1:]
            )
        ...
        if speech_emb.shape[0] != text_emb.shape[0]:
            speech_emb = mx.broadcast_to(
                speech_emb,
                (text_emb.shape[0],) + speech_emb.shape[1:]
            )
    ...
    def __call__(
        ...
        for i in range(B):
            ...
            text_latents = mx.concatenate([
                text_latents[:i],
                hidden_states[i:i+1, text_start:text_end, :],
                text_latents[i+1:],
            ], axis=0) if i < B else text_latents

            speech_latents = mx.concatenate([
                speech_latents[:i],
                hidden_states[i:i+1, speech_start:speech_end, :],
                speech_latents[i+1:],
            ], axis=0) if i < B else speech_latents
    ...
    def inference(
        ...
        for step in range(max_new_tokens):
            ...
            # For CFG, duplicate
            if cfg_weight > 0.0:
                next_token_embed = mx.concatenate([next_token_embed, next_token_embed], axis=0)
