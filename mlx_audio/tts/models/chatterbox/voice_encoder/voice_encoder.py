...
def sanitize_lstm_weights(key: str, value: mx.array, num_layers: int = 3) -> Dict[str, mx.array]:
    """
...
class VoiceEncoder(nn.Module):
    ...
    def sanitize(self, weights: Dict[str, mx.array]) -> Dict[str, mx.array]:
        ...
        for key, value in weights.items():
            # Handle LSTM weights
            if "lstm." in key and any(x in key for x in ["weight_ih", "weight_hh", "bias_ih", "bias_hh"]):
                import re
                match = re.search(r'lstm\.(weight_ih|weight_hh|bias_ih|bias_hh)_l(\d+)', key)
                if match:
                    weight_type = match.group(1)
        ...
        **kwargs
        ...
        if trim_top_db is not None:
            ...
            for wav in wavs:
                ...
                if n_frames > 0:
                    rms = np.array([
                        math.sqrt(np.mean(wav_np[i * hop_length:i * hop_length + frame_length] ** 2))
                    ...
                    if len(non_silent) > 0:
                        ...
                        end_sample = min(end_frame * hop_length + frame_length, len(wav_np))
